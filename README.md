...
| Thumb | The wide thumbnail used in some list and resume views. |
...
the build number in `src/Emby.Plugin.CustomArtFetcher/build-number.txt` supplies the fourth, incrementing
each time — so the version Emby shows on the plugin page always identifies the exact DLL you
installed. The build prints it (`Emby.Plugin.CustomArtFetcher version 1.1.0.13`). Bump `VersionBase` by hand
...
- Only the primary image for movies and series is handled. Backdrops/Thumbnails, episodes and seasons,
